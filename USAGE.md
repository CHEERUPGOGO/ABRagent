...
### 方式二：启动 Web 监控大屏 (FastAPI 只读版)
```bash
# 启动 Web 监控大屏 (浏览器打开 http://127.0.0.1:7865)
python auto_battery_research_cli.py --web

# 旧版交互式 Gradio 仪表盘 (后备)
python auto_battery_research_cli.py --web-gradio
```
...
| `auto_battery_research_cli.py --web` | **FastAPI Web 监控大屏 (只读)** | Web 浏览器操作 | 课题列表、阶段进度、研报渲染、运行日志 | `python auto_battery_research_cli.py --web` |
