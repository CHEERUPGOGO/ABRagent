#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RAG 问答前端 — v3 优化版 (qwen3-embedding:8b / Ollama GPU)：组件过滤 + top-15 + 标签加权排序

相对 v3 的改动：
  - 嵌入模型: bge-m3 → qwen3-embedding:8b (Ollama/GPU)
  - Chroma 目录/collection 指向 qwen 向量库
"""
...
CHROMA_DIR = "/home/ls/xiaoyue/LLM2/LMLLM/miner/chroma/paragraphs_v5_qwen"
COLLECTION = "battery_paragraphs_v5_qwen"
TOP_K = 10
SEARCH_K = 15
...
class RAG:
    ...
    def ask(self, question, top_k=TOP_K):
        ...
        info = f"[Qwen3-Emb | 组件={comp or '无'} | 标签={'/'.join(expected_labels) if expected_labels else '无'} | 召回{len(docs)}/{len(scored)}段 | 检索置信度:{conf_pct}%]"
...
demo = gr.ChatInterface(respond,
    title="锂电池文献 RAG (v3优化·Qwen3-Embedding-8B GPU)",
    description="Qwen3-Embedding-8B (GPU) + top-15 搜索 + 标签加权排序",
