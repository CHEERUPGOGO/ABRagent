#!/usr/bin/env python3
"""电解质 agent 完整管线：Condition → Material → Performance 三agent联动"""
...
logger = logging.getLogger("ElectrolytePipeline")

def _load_mod(fp):
    s = importlib.util.spec_from_file_location(Path(fp).stem, fp)
    m = importlib.util.module_from_spec(s); s.loader.exec_module(m); return m

_pm = _load_mod(str(_PROJECT_ROOT/"miner"/"electrolyte_database"/"electrolyte_prompts1.py"))
P_MAT_INC=_pm.PROMPT_ELECTROLYTE_MATERIAL_INCLUDE; P_MAT_EXT=_pm.PROMPT_ELECTROLYTE_MATERIAL_EXTRACT
P_PERF_INC=_pm.PROMPT_ELECTROLYTE_PERFORMANCE_INCLUDE; P_PERF_EXT=_pm.PROMPT_ELECTROLYTE_PERFORMANCE_EXTRACT
P_COND_INC=_pm.PROMPT_ELECTROLYTE_CONDITION_INCLUDE; P_COND_EXT=_pm.PROMPT_ELECTROLYTE_CONDITION_EXTRACT; P_COND_EX=_pm.CONDITION_EXAMPLE_TEXT


class ElectrolyteMaterialAgent1(BaseAgentV2):
    formatter:Any=None; prompt_include:str=P_MAT_INC; prompt_extract:str=P_MAT_EXT; base_name:str="electrolyte-material"
...
class ElectrolytePerformanceAgent1(BaseAgentV2):
    formatter:Any=None; prompt_include:str=P_PERF_INC; prompt_extract:str=P_PERF_EXT; base_name:str="electrolyte-perf"
    def _call(self,inputs,rm=None):
...
class ElectrolyteConditionAgent(Chain):
...
def discover_materials(full_text: str, file_stem: str, inc_llm):
    """使用 MaterialDiscoveryAgent 识别论文中的电解质材料"""
    ...
    mats = agent.discover(raw, component="electrolyte", file_stem=file_stem)
    if not mats:
        tm = re.search(r'#\s+(.+)', raw or "")
        name = tm.group(1).strip()[:80] if tm else "electrolyte"
...
def process_one_material(m, i, paras, doi, global_cond, global_cond_items, primary_cid):
    ...
    mat = ElectrolyteMaterialAgent1.from_llm(il, el, tc); mat.formatter = ElectrolyteFormatter()
    perf = ElectrolytePerformanceAgent1.from_llm(il, el, tc); perf.formatter = ElectrolyteFormatter()
...
def run(fp,od):
    ...
    cnd0=ElectrolyteConditionAgent.from_llm(il,el,tc); gc=[]; gci=[]
    for p in paras:
        o=si(cnd0,{"content":p,"material_id":"global","battery_system_context":"electrolyte","known_conditions":gc})
    ...
    for mode,data,name in [("electrolyte-material",am,"electrolyte_material"),("electrolyte-perf",ap,"electrolyte_perf"),("electrolyte-cond",ac,"electrolyte_cond")]:
    ...
    j={"file":fp,"doi":doi,"title":title,"agent_mode":"electrolyte-pipeline","materials":materials,"systems":cb,"global_conditions":gc,"global_condition_id":pcid}
    with open(os.path.join(od,f"{st}_electrolyte_pipeline.json"),"w",encoding="utf-8") as f: json.dump(j,f,ensure_ascii=False,indent=2)
    print(f"  -> {st}_electrolyte_pipeline.json")
    print(f"\nToken: {tc.summary()}")
    print(f"\n✅ done -> {od}")

if __name__=="__main__":
    run(str(_PROJECT_ROOT/"papers"/"text_merged"/"mergemarkdown5"/"10.1021_acsnano.3c02948.md"),str(_PROJECT_ROOT/"miner"/"json"))
