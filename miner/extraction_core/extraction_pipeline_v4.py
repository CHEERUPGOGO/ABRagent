# -*- coding: utf-8 -*-
"""提取流水线 v4 — 文件级 + 材料级双重并行 — 并行版：Phase 0(材料识别) → 全局条件 → 按材料并行处理 → TableAgent
...
def _process_one_material(mat, idx, paragraphs, doi, discovery_agent, cond_agent,
    ...
    # Build conditions_index for quick lookup
    conditions_index = {}
    for item in all_items:
        for c in item.get("conditions", []):
            cid = c.get("condition_id", "")
            if cid:
                conditions_index[cid] = c
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="v4 并行提取流水线（文件级 + 材料级并行）")
    ...
    p.add_argument("--workers", type=int, default=4, help="每文件内材料并行线程数")
    p.add_argument("--file-workers", type=int, default=3, help="文件间并行线程数")
    ...
    logger.info(f"{len(tasks)} tasks | components={list(registry.keys())} | file_workers={args.file_workers}")

    # 文件级并行锁（TokenChecker 非线程安全）
    import threading
    ...
    def _process_one_file(task):
        ...
        fp = task["file_path"]
        try:
            res = process_file(fp, registry[comp], comp, meta_index, max_workers=args.workers)
            base = os.path.splitext(os.path.basename(fp))[0]
            out_path = os.path.join(odir, f"{base}_{comp}_extracted_v4.json")
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(res, f, ensure_ascii=False, indent=2)
            return res
        except Exception as e:
            logger.error(f"Error [{comp}] {os.path.basename(fp)}: {e}")
            return {"file": fp, "comp": comp, "error": str(e)}

    all_results = []
    with ThreadPoolExecutor(max_workers=min(args.file_workers, len(tasks))) as pool:
        futures = {pool.submit(_process_one_file, t): t for t in tasks}
        for f in as_completed(futures):
            t = futures[f]
            comp = t["component"]
            logger.info(f"  [完成] {comp}: {os.path.basename(t['file_path'])}")
            try:
                r = f.result()
                all_results.append(r)
            except Exception as e:
                logger.error(f"Submit error [{comp}]: {e}")
                all_results.append({"file": t["file_path"], "error": str(e)})

    summary = os.path.join(odir, "_pipeline_v4_summary.json")
    with open(summary, "w", encoding="utf-8") as f:
        json.dump({"pipeline_version": "v4", "n_tasks": len(all_results),
                   "token_summary": tc.summary() if tc else {}, "results": all_results},
                  f, ensure_ascii=False, indent=2)
    ok = sum(1 for r in all_results if "error" not in r)
    print(f"\n✅ [v4] {ok}/{len(tasks)} files -> {odir}")
