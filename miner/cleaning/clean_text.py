...
def get_section_status(header_text: str, mode: str = "extract") -> Optional[str]:
    ...
    classify 模式：保留标题/摘要/引言，遇到实验/结果等正文章节即停止。
...
def clean_and_merge_lines(lines: List[str], mode: str = "extract") -> List[str]:
    ...
    is_first_heading = True  # 第一个 # 标题（论文标题）不匹配章节关键词
    ...
    for line in lines:
        ...
        if in_header_noise:
            ...
            # 段落合并逻辑
            # 空行后新段落：检查是否与上一段为断句（前段无句号 + 本段小写开头）
            if not current_buffer:
                if cleaned_paragraphs:
                    prev = cleaned_paragraphs[-1]
                    ends_with_punct = prev.rstrip()[-1] in ('.', '!', '?') if prev.rstrip() else False
                    starts_with_lower = curr_text[0].islower() if curr_text else False
