#!/usr/bin/env python3
"""v5 — v4 基础上修复表格合并 + 超长段落

相对 v4 的改动：
  - split_paragraphs 合并时检测表格边界 (<table>(Continued))
  - 表格与正文互不粘合，消除超长段问题
  - 表格块之间也不合并，保持独立
"""
...
_REF_PATTERN = re.compile(r"^\d+\.\s+[A-Z]")
...
def collect_paragraphs(input_root: Path, min_length: int, limit: Optional[int],
                       ...
                       incremental: bool = False) -> List[ParagraphItem]:
...
def write_json(records: List[dict], output_json: Path) -> None:
    ...
    # 如果文件已存在，读取旧记录并合并（去重键: source_paper + paragraph_context）
    if output_json.exists():
        with open(output_json, encoding="utf-8") as f:
            existing_records = json.load(f)
        seen = {(r.get("source_paper"), r.get("paragraph_context")) for r in existing_records}
        for r in records:
            ...
            if key not in seen:
                existing_records.append({
                    ...
                    "label": r["label"],
                    "paragraph_context": r["paragraph_context"],
                })
...
def main() -> None:
    ...
    # 标签后、入库前：过滤非正文 + 超长段双存（子段检索命中 → 返回整段）
    MAX_CHUNK = 7500
    OVERLAP = 200
    splitted = []
    for rec in records:
        ...
        if len(text) <= MAX_CHUNK or text.startswith("<table"):
            splitted.append(rec)
            continue
        # 超长段：先存整段入口(用于返回)，再存子段(用于检索)
        base_id = rec.get("_id", "unknown")
        full_rec = dict(rec)
        full_rec["paragraph_context"] = text  # 完整文本，向量由前 MAX_CHUNK 字决定
