#!/usr/bin/env python3
"""正极 agent 完整管线：Condition → Material → Performance 三agent联动"""
...
logger = logging.getLogger("CathodePipeline")

def _load_mod(fp):
    s = importlib.util.spec_from_file_location(Path(fp).stem, fp)
    m = importlib.util.module_from_spec(s); s.loader.exec_module(m); return m

_pm = _load_mod(str(_PROJECT_ROOT/"miner"/"cathode_database"/"cathode_prompts1.py"))
P_MAT_INC=_pm.PROMPT_CATHODE_MATERIAL_INCLUDE; P_MAT_EXT=_pm.PROMPT_CATHODE_MATERIAL_EXTRACT
P_PERF_INC=_pm.PROMPT_CATHODE_PERFORMANCE_INCLUDE; P_PERF_EXT=_pm.PROMPT_CATHODE_PERFORMANCE_EXTRACT
P_COND_INC=_pm.PROMPT_CATHODE_CONDITION_INCLUDE; P_COND_EXT=_pm.PROMPT_CATHODE_CONDITION_EXTRACT; P_COND_EX=_pm.CONDITION_EXAMPLE_TEXT


class CathodeMaterialAgent1(BaseAgentV2):
    formatter:Any=None; prompt_include:str=P_MAT_INC; prompt_extract:str=P_MAT_EXT; base_name:str="cathode-material"
...
class CathodePerformanceAgent1(BaseAgentV2):
    formatter:Any=None; prompt_include:str=P_PERF_INC; prompt_extract:str=P_PERF_EXT; base_name:str="cathode-perf"
    def _call(self,inputs,rm=None):
...
class CathodeConditionAgent(Chain):
...
def discover_materials(full_text: str, file_stem: str, inc_llm):
    """使用 MaterialDiscoveryAgent 识别论文中的正极材料"""
    ...
    mats = agent.discover(raw, component="cathode", file_stem=file_stem)
    if not mats:
        tm = re.search(r'#\s+(.+)', raw or "")
        name = tm.group(1).strip()[:80] if tm else "cathode material"
...
def process_one_material(m, i, paras, doi, global_cond, global_cond_items, primary_cid):
    ...
    mat = CathodeMaterialAgent1.from_llm(il, el, tc); mat.formatter = CathodeFormatter()
    perf = CathodePerformanceAgent1.from_llm(il, el, tc); perf.formatter = CathodeFormatter()
...
def run(fp,od):
    ...
    cnd0=CathodeConditionAgent.from_llm(il,el,tc); gc=[]; gci=[]
    for p in paras:
        o=si(cnd0,{"content":p,"material_id":"global","battery_system_context":"cathode","known_conditions":gc})
    ...
    for mode,data,name in [("cathode-material",am,"cathode_material"),("cathode-perf",ap,"cathode_perf"),("cathode-cond",ac,"cathode_cond")]:
    ...
    j={"file":fp,"doi":doi,"title":title,"agent_mode":"cathode-pipeline","materials":materials,"systems":cb,"global_conditions":gc,"global_condition_id":pcid}
    with open(os.path.join(od,f"{st}_cathode_pipeline.json"),"w",encoding="utf-8") as f: json.dump(j,f,ensure_ascii=False,indent=2)
    print(f"  -> {st}_cathode_pipeline.json")
    print(f"\nToken: {tc.summary()}")
    print(f"\n✅ done -> {od}")

if __name__=="__main__":
    run(str(_PROJECT_ROOT/"papers"/"text_merged"/"mergemarkdown5"/"10.1038_s41467-024-54637-9.md"),str(_PROJECT_ROOT/"miner"/"json"))
